...
Versiyon: 4.0.0 (Ultra-Premium Master Build)
Geliştirici: Stellaris IT & UI/UX Ekibi
Açıklama: Ultra Yüksek Net Değere Sahip Bireyler (UHNWI) için tasarlanmış, 
interaktif 3D simülasyonlar, canlı uydu verileri ve çok adımlı rezervasyon sistemi 
içeren tam teşekküllü Streamlit Web Portalı.
=========================================================================================
"""
...
# =======================================================================================
st.set_page_config(
    page_title="Stellaris | Premium Astro-Tourism", 
    page_icon="🌌", 
    layout="wide", 
    initial_sidebar_state="expanded"
)

# Kapsamlı ve Detaylı CSS Enjeksiyonu
st.markdown("""
    <style>
    ...
    /* Genel Kaydırma Çubuğu Optimizasyonu */
    ...
    /* Kozmik Arka Plan ve Katmanlı Animasyon */
    .stApp { 
    ...
    /* Radyo Butonları (Yan Menü Navigasyonu) Özelleştirmesi */
    div[role="radiogroup"] > label > div:first-of-type { display: none !important; }
    div[role="radiogroup"] p { 
        color: #B8860B !important; font-family: 'Cinzel', serif !important; font-size: 1.05rem !important; 
        font-weight: 600 !important; text-align: center !important; visibility: visible !important; 
        display: block !important; width: 100%; margin-top: 5px; transition: all 0.4s ease; 
        padding: 10px 0; border-radius: 6px; letter-spacing: 1px;
    }
    div[role="radiogroup"] label:hover p { 
        color: #D4AF37 !important; text-shadow: 0px 0px 10px rgba(212, 175, 55, 0.5); 
        background: rgba(184, 134, 11, 0.1); transform: translateX(8px);
    }
    div[role="radiogroup"] label[aria-checked="true"] p { 
        color: #FFD700 !important; text-shadow: 0px 0px 15px rgba(255, 215, 0, 0.8); 
        border-bottom: 2px solid #B8860B; background: rgba(184, 134, 11, 0.2); transform: scale(1.02);
    }

    /* Girdi Elementleri Özelleştirmesi */
    [data-baseweb="select"], input[type="text"], input[type="password"] { 
        background-color: rgba(3, 8, 20, 0.6) !important; 
        border: 1px solid #B8860B !important; 
        border-radius: 6px; backdrop-filter: blur(5px); color: #FFF !important;
    }
    [data-baseweb="select"] * { color: #C5A059 !important; font-family: 'Montserrat', sans-serif !important; }
    
    /* Metrik (İstatistik) Kartları */
    div[data-testid="stMetricValue"] { 
        color: #D4AF37 !important; font-family: 'Cinzel', serif !important; 
        font-size: 2.8rem !important; text-shadow: 0px 0px 20px rgba(212, 175, 55, 0.6); 
        transition: transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); display: inline-block;
    }
    div[data-testid="stMetricValue"]:hover { transform: scale(1.15) translateY(-5px); color: #FFD700 !important; }
    ...
    div[data-testid="stMetricDelta"] svg { fill: #00e676 !important; color: #00e676 !important; }
    
    /* Dinamik Başlıklar (H1-H6) */
    h1, h2, h3, h4, h5, h6 { 
        font-family: 'Cinzel', serif; font-weight: 700; text-align: center !important; width: 100%; 
    ...
    @keyframes pulseGlow { 
        ...
        to { text-shadow: 0 0 30px rgba(212,175,55,0.7); } 
    }
    ...
    .hero-image { 
        width: 100%; max-width: 1400px; height: 60vh; object-fit: cover; border-radius: 12px; 
        filter: brightness(65%); border: 1px solid rgba(184, 134, 11, 0.5); 
        animation: slowZoom 40s infinite alternate cubic-bezier(0.455, 0.03, 0.515, 0.955); 
    }
    ...
    /* Premium Servis Kartları, Gezegen Kartları ve İstatistik Kutuları */
    .service-card, .planet-card, .stat-box, .info-panel { 
        background: rgba(5, 16, 36, 0.45) !important; backdrop-filter: blur(15px) !important; 
        border-radius: 16px !important; border: 1px solid rgba(184, 134, 11, 0.3) !important; 
        box-shadow: 0 10px 40px 0 rgba(0, 0, 0, 0.6) !important; 
        transition: all 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) !important; overflow: hidden; 
    }
    .service-card:hover, .planet-card:hover, .stat-box:hover, .info-panel:hover { 
    ...
    div.stButton > button:first-child { 
        background: linear-gradient(135deg, rgba(3,8,20,0.95) 0%, rgba(17,34,64,0.95) 100%) !important; 
        color: #D4AF37 !important; font-family: 'Montserrat', sans-serif; font-weight: 600; 
        font-size: 1.2rem; padding: 18px 60px; border: 1px solid rgba(184, 134, 11, 0.6) !important; 
        border-radius: 40px; transition: all 0.4s ease; letter-spacing: 3px; text-transform: uppercase; 
        box-shadow: 0 5px 20px rgba(0,0,0,0.6);
    }
    div.stButton > button:first-child:hover { 
        background: linear-gradient(135deg, rgba(184, 134, 11, 0.25) 0%, rgba(212, 175, 55, 0.4) 100%) !important; 
        color: #FFF !important; border-color: #D4AF37 !important; transform: translateY(-4px) scale(1.03); 
    ...
    [data-baseweb="tab"] { 
        background: rgba(5, 16, 36, 0.7) !important; color: #8892b0 !important; 
        font-family: 'Cinzel', serif; font-size: 1rem; padding: 12px 25px; 
        border-radius: 30px; border: 1px solid rgba(184,134,11,0.2) !important; transition: all 0.3s ease;
    }
    [aria-selected="true"] { 
        background: rgba(184, 134, 11, 0.2) !important; color: #FFD700 !important; 
        border-color: #D4AF37 !important; box-shadow: 0 0 20px rgba(184,134,11,0.3); transform: scale(1.05);
    }
    ...
    .spinning-planet { 
        width: 280px; height: 280px; border-radius: 50%; display: block; margin: 0 auto 25px auto; 
        box-shadow: inset -30px -30px 50px rgba(0,0,0,0.95), 0 0 40px rgba(184, 134, 11, 0.4); 
        animation: spin 45s linear infinite; object-fit: cover;
    }
    @keyframes spin { 100% { transform: rotate(360deg); } }
    @keyframes blink { 0% { opacity: 1; } 50% { opacity: 0.3; } 100% { opacity: 1; } }
    
    /* Kurumsal Misyon Kutuları */
    .mission-box { 
        background: rgba(3, 8, 20, 0.75); border-left: 5px solid #D4AF37; padding: 35px; 
        margin-bottom: 30px; border-radius: 0 12px 12px 0; text-align: left !important; 
        box-shadow: 0 8px 20px rgba(0,0,0,0.5); transition: transform 0.3s ease;
    }
    ...
    /* Dataframe ve Tablo Optimizasyonları */
    [data-testid="stDataFrame"] { border: 1px solid rgba(184, 134, 11, 0.4); border-radius: 8px; }
    </style>
...
if "booking_step" not in st.session_state:
    st.session_state.booking_step = 1 # Çok adımlı rezervasyon için
...
if st.session_state.logged_in:
    st.sidebar.markdown(f"<div style='text-align:center; color:#D4AF37; font-family:Montserrat; margin-bottom:10px; font-size: 1.1rem;'><b>👤 {st.session_state.current_user}</b></div>", unsafe_allow_html=True)

st.sidebar.write("---")

# Ana Menü (Ciddi ve Kurumsal)
menu_secenekleri = [
...
# Müzik Çalar (Kalıcı ve Kesintisiz)
...
# Yardımcı Fonksiyon: Canlı Uydu Verisi Formatlayıcı
def auto_refresh_image(img_id, img_url, refresh_rate_ms, title, max_width="600px", filter_css="none"):
    """NASA/NOAA uydu görüntülerini iframe engeline takılmadan JS ile canlı yeniler."""
    return f"""
...
# =======================================================================================
# 4. SAYFA İÇERİKLERİ VE İŞ MANTIKLARI (BUSINESS LOGIC)
# =======================================================================================

# ---------------------------------------------------------------------------------------
# SAYFA 1: ANA SAYFA (KURUMSAL VİTRİN)
# ---------------------------------------------------------------------------------------
if menu_secimi in ["ANA SAYFA", "HOME"]:
    ...
    # Premium Holding İstatistikleri
...
# ---------------------------------------------------------------------------------------
# SAYFA 2: LOKASYONLARIMIZ (3 FARKLI KITA)
# ---------------------------------------------------------------------------------------
    ...
    # Veri Tabanı Mantığıyla Lokasyonlar
    locations = [
        {
        ...
        {
            "img": "https://images.unsplash.com/photo-1610313886566-50d4f3b50c05?q=80&w=800",
...
# ---------------------------------------------------------------------------------------
# SAYFA 3: CANLI GÖZLEMEVİ (NASA/NOAA HAM VERİ)
# ---------------------------------------------------------------------------------------
    ...
    with tab7:
        ...
        with col_controls:
            ...
            zoom_level = st.slider("Zoom / Yakınlaştırma", 1.0, 4.0, 1.0, 0.1)
...
# ---------------------------------------------------------------------------------------
    ...
    # NASA API'leri Sabitlendi (allowfullscreen eklendi)
    ...
    with tab5:
        loc_choice = st.selectbox("Gözlem Noktası Seçimi / Select Observation Point:", ["Atacama Çölü, Şili", "Tekapo Gölü, Yeni Zelanda", "NamibRand, Namibya"])
        if "Atacama" in loc_choice: lat, lon = -23.0, -67.7
        elif "Tekapo" in loc_choice: lat, lon = -44.0, 170.4
        else: lat, lon = -25.2, 15.9 # Namibia
...
# ---------------------------------------------------------------------------------------
# SAYFA 5: KARA DELİK FİZİK MOTORU (HTML5 CANVAS)
# ---------------------------------------------------------------------------------------
    ...
    with col2:
        ...
        <div style="border: 1px solid rgba(184,134,11,0.3); border-radius: 12px; overflow: hidden; background: rgba(5,16,36,0.6); backdrop-filter: blur(10px); padding: 30px; box-shadow: 0 10px 40px rgba(0,0,0,0.5);">
            ...
            <canvas id="bhCanvas" width="800" height="450" style="display: block; margin: 0 auto; background: radial-gradient(circle at center, #0a1930 0%, #000 100%); border-radius: 8px; border: 1px solid rgba(184,134,11,0.2); box-shadow: 0 0 30px rgba(0,0,0,0.8) inset;"></canvas>
        </div>
        <script>
            ...
            function animate() {{
                ...
                // Physics calculation for Accretion Disk
                particles.forEach(p => {{
...
# ---------------------------------------------------------------------------------------
# SAYFA 7: KOZMİK TAKVİM (KIRIK GÖRSELLER WIKIMEDIA İLE DEĞİŞTİRİLDİ)
# ---------------------------------------------------------------------------------------
    ...
    with col_info:
        # Wikipedia Resim Bağlantıları - Kalıcı ve Sağlam
    ...
    with col_metrics: 
        ...
        st.write("---")
        st.metric("Aktivite Seviyesi / Activity Level", "Yüksek / High", "Max")
...
# ---------------------------------------------------------------------------------------
# SAYFA 8: UZAY HAVADURUMU (GELİŞMİŞ DASHBOARD)
# ---------------------------------------------------------------------------------------
    ...
    # Çoklu Metrik Ekranı
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.markdown(f"<div class='stat-box'><div class='stat-number' style='font-size:2.2rem;'>6.33</div><div class='stat-label'>Kp Index</div></div>", unsafe_allow_html=True)
    with c2: st.markdown(f"<div class='stat-box'><div class='stat-number' style='font-size:2.2rem;'>540</div><div class='stat-label'>{'Güneş Rüzgarı (km/s)' if lang=='TR' else 'Solar Wind (km/s)'}</div></div>", unsafe_allow_html=True)
    with c3: st.markdown(f"<div class='stat-box'><div class='stat-number' style='font-size:2.2rem;'>%85</div><div class='stat-label'>{'Aurora İhtimali' if lang=='TR' else 'Aurora Prob.'}</div></div>", unsafe_allow_html=True)
    with c4: st.markdown(f"<div class='stat-box'><div class='stat-number' style='font-size:2.2rem;'>M1.2</div><div class='stat-label'>{'Parlama Sınıfı' if lang=='TR' else 'Solar Flare'}</div></div>", unsafe_allow_html=True)
    
    st.write("---")
    
    # 2 Farklı İstatistik Grafiği
...
# ---------------------------------------------------------------------------------------
# SAYFA 9: IŞIK KİRLİLİĞİ (GELİŞMİŞ EFEKTLER)
# ---------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------
# SAYFA 10: EKİPMAN KATALOĞU (RESİMSİZ, TAMAMEN PROFESYONEL VE CİDDİ HTML)
# ---------------------------------------------------------------------------------------
    ...
    with col1:
        ...
        <div class="service-card" style="padding:30px; text-align:left; height:auto; margin-bottom:25px;">
            ...
            <b style="color:#B8860B;">{use_str}:</b> {"Tam otomatik 16 kanallı GPS hizalama sistemi ve StarBright XLT kaplaması ile %97 maksimum ışık geçirgenliği sağlar. Derin uzay (Deep-Sky) galaksi ve bulutsu (Nebula) analizleri için tasarlanmıştır." if lang=='TR' else "Provides 97% maximum light transmission with fully automated 16-channel GPS alignment and StarBright XLT coating. Designed for deep-sky galaxy and nebula analysis."}</p>
        </div>
        <div class="service-card" style="padding:30px; text-align:left; height:auto; margin-bottom:25px;">
            <h3 style="color:#D4AF37; margin-bottom:15px; text-align:left !important; border-bottom: 1px solid rgba(184,134,11,0.3); padding-bottom:10px;">Lunt LS100MT Solar Observatory</h3>
            <p style="text-align:left !important; color:#E0E0E0; line-height:2;">
            <b style="color:#B8860B;">{type_str}:</b> Dedicated H-alpha Solar<br>
            <b style="color:#B8860B;">{aper_str}:</b> 100mm (Double Stacked)<br>
            <b style="color:#B8860B;">{use_str}:</b> {"Güneş rüzgarlarını, C-sınıfı patlamaları ve taçküreyi (corona) gerçek zamanlı ve %100 güvenli izlemek için çift yığınlı endüstri standardı filtreleme mekanizması kullanır." if lang=='TR' else "Utilizes double-stacked industry standard filtering mechanisms to view solar winds, C-class flares, and the corona in real-time with 100% safety."}</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"""
        <div class="service-card" style="padding:30px; text-align:left; height:auto; margin-bottom:25px;">
            <h3 style="color:#D4AF37; margin-bottom:15px; text-align:left !important; border-bottom: 1px solid rgba(184,134,11,0.3); padding-bottom:10px;">ZWO ASI 6200MM Pro (Cooled)</h3>
            <p style="text-align:left !important; color:#E0E0E0; line-height:2;">
            <b style="color:#B8860B;">{"Sensör Mimarisi" if lang=="TR" else "Sensor Arch"}:</b> Full Frame CMOS (Monochrome)<br>
            <b style="color:#B8860B;">{"Sensör Çözünürlüğü" if lang=="TR" else "Sensor Res."}:</b> 62 Megapixels<br>
            <b style="color:#B8860B;">{use_str}:</b> {"İki aşamalı termoelektrik soğutma (-35°C delta T) sistemi sayesinde karanlık akım parazitini (Dark Current Noise) sıfıra indirir. Astrofotografi workshoplarında misafirlerimize kullandırılır." if lang=='TR' else "Reduces Dark Current Noise to zero thanks to its two-stage thermoelectric cooling (-35°C delta T) system. Provided to guests during astrophotography workshops."}</p>
        </div>
        <div class="service-card" style="padding:30px; text-align:left; height:auto;">
            <h3 style="color:#D4AF37; margin-bottom:15px; text-align:left !important; border-bottom: 1px solid rgba(184,134,11,0.3); padding-bottom:10px;">Software Bisque Paramount ME II</h3>
            <p style="text-align:left !important; color:#E0E0E0; line-height:2;">
            ...
            <b style="color:#B8860B;">{use_str}:</b> {"Devasa teleskop dizilerini titreşimsiz ve Dünya'nın dönüş hızıyla milimetrik düzeyde senkronize hareket ettiren, TheSkyX Professional yazılımı ile donatılmış endüstriyel taşıyıcı ayak." if lang=='TR' else "An industrial mount equipped with TheSkyX Professional software that moves massive telescope arrays smoothly and in millimeter synchronization with the Earth's rotation speed."}</p>
        </div>
        """, unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------------------
    ...
    st.markdown("<h2>{}</h2>".format("Astro-Fotoğrafçılık Simülatörü" if lang == "TR" else "Astro-Photography Simulator"), unsafe_allow_html=True)
    st.write("---")
    col_ctrl, col_view = st.columns([1, 2])
    with col_ctrl: 
        st.markdown(f"<div class='info-panel' style='padding:20px; height:100%;'>", unsafe_allow_html=True)
    ...
    with col_view: 
        ...
        # ISO yüksekse karıncalanma (noise) efekti vermek için contrast ve sepia ayarları yapılabilir.
...
# ---------------------------------------------------------------------------------------
# SAYFA 12: VIP REZERVASYON (ÇOK ADIMLI E-TİCARET ALTYAPISI)
# ---------------------------------------------------------------------------------------
    ...
    with col_center:
        ...
        # Seçimler
        deneyim_turu = st.radio("Ana Paket Seçimi / Select Main Package:", [
        ...
        ekstra1 = st.checkbox("Kişisel Astrofotografçı (ZWO Kameralar Dahil) / Personal Astrophotographer (+$2,500)")
        ekstra2 = st.checkbox("Özel Helikopter ile Havalimanı Transferi / Private Heli-Transfer (+$4,000)")
        ekstra3 = st.checkbox("VIP Uzay Menüsü (Michelin Yıldızlı Şef) / Michelin Star Private Chef (+$3,000)")
        
        st.write("---")
        col_form1, col_form2 = st.columns(2)
        with col_form1: secilen_tarih = st.date_input("Tahmini Geliş Tarihi / Estimated Date", min_value=datetime.date.today())
        ...
        ekstra_toplam = (2500 if ekstra1 else 0) + (4000 if ekstra2 else 0) + (3000 if ekstra3 else 0)
        ...
        if st.button("Talebi İlet / Submit Request", use_container_width=True): 
            st.toast("Talebiniz elit danışmanlarımıza iletilmiştir. Özel jetiniz hazırlanıyor..." if lang == "TR" else "Request sent to our elite advisors. Private jet is prepping...", icon="🥂")
            st.balloons()
        st.markdown("</div>", unsafe_allow_html=True)

# ---------------------------------------------------------------------------------------
# SAYFA 13: VİZYON & SÜRDÜRÜLEBİLİRLİK (KURUMSAL ŞİRKET PROFİLİ)
# ---------------------------------------------------------------------------------------
    ...
    t_vis, t_sus, t_exo = st.tabs(["🌟 Vizyon & Misyon" if lang == "TR" else "🌟 Vision & Mission", "🍃 Sürdürülebilirlik" if lang == "TR" else "🍃 Sustainability", "🚀 Uzay Biletlemesi (2040)" if lang == "TR" else "🚀 Space Ticketing (2040)"])
...
# ---------------------------------------------------------------------------------------
# SAYFA 14: YATIRIMCI PORTALI (GENİŞLETİLMİŞ FİNANSAL RAPORLAMA)
# ---------------------------------------------------------------------------------------
    ...
    if st.text_input("Yetkilendirme Şifresi / Authorization Key (stellaris2026):", type="password") == "stellaris2026": 
        st.success("Güvenli Bağlantı Kuruldu. / Secure Connection Established.")
        
        # Üst Düzey Finansal Metrikler
        c1, c2, c3 = st.columns(3)
        ...
        with col_c2:
            ...
            st.line_chart(df_occ, color=["#FFD700", "#ffffff"])
        ...
        <div class="info-panel" style='text-align:left !important; padding:30px;'>
            ...
            {'Gelecek yıl Afrika kıtasında, Namibya Çölü (NamibRand) bölgesinde planlanan 3. faz VIP rezervasyon ve konaklama alanı inşası ile küresel astro-turizm pazar payımızın %65 oranında artması öngörülmektedir. Elon Musk ve SpaceX yöneticileriyle yapılan ön görüşmeler, gelecekteki Mars simülasyon eğitimlerinin tesislerimizde yapılması üzerine yoğunlaşmıştır.' if lang=='TR' else 'With the 3rd phase VIP booking and accommodation area planned in the Namib Desert (NamibRand) on the African continent next year, our global astro-tourism market share is projected to increase by 65%. Preliminary talks with Elon Musk and SpaceX executives have focused on conducting future Mars simulation trainings at our facilities.'}
            </p>
        </div>
        """, unsafe_allow_html=True)
